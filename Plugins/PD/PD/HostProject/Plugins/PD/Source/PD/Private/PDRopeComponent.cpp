	...
	:Super(ObjectInitializer)
{
	...
	NumSegmentsV = 30;
	NumSidesV = 4;
	EndLocationV = FVector(100.f, 0, 0);
	RopeLengthV = 100.f;
	SubstepTimeV = 0.02f;
	SolverIterationsV = 1;
	TileMaterialV = 1.f;
...
void UPDRopeComponent::GetEndPositions(FVector& OutStartPosition, FVector& OutEndPosition) {
	...
	USceneComponent* EndComponent = Cast<USceneComponent>(AttachEndToV.GetComponent(GetOwner()));
...
void UPDRopeComponent::WritebackClothingSimulationData()
{
