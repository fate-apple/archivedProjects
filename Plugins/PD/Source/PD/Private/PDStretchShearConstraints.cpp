	...
	const PDScalar Dt,const int InConstraintIndex,SparseMatrix&CRPosLhs ,SparseMatrix&CRQuatLhs,Vec&CRPosRhs,Vec&CRQuatRhs)
{
	const int32 i = InConstraintIndex;
	{
		...
		diff_un = Quat::FromTwoVectors(xf, d3).normalized();
		un_star = Q1*diff_un;
		...
		auto debug1 = EigenMatrix2StdVector(CRPosLhs);
	...
	(Vec& NewPos,Vec& NewQuat,SparseMatrix&CRPosLhs ,SparseMatrix&CRQuatLhs,Vec&CRPosRhs,Vec&CRQuatRhs,bool bInitLhs )
{
	...
	PosSelectionMatrix =  SparseMatrix(6, NewPos.rows());
	PosSelectionMatrix.setZero();
...
void FPDStretchShearConstraints::computeProjection(Vec& NewPos,Vec& NewQuat,
	const int InConstraintIndex,SparseMatrix&CRPosLhs ,SparseMatrix&CRQuatLhs,Vec&CRPosRhs,Vec&CRQuatRhs)
{
	const int32 i = InConstraintIndex;
	{
		...
		const int32 i2 = Constraint[1];
		const int32 e1 = Constraint[2];
		TVector<PDScalar,3>  P1 = TVector<PDScalar,3>(NewPos[3*i1],NewPos[3*i1+1],NewPos[3*i1+2]);
		TVector<PDScalar,3>  P2 = TVector<PDScalar,3>(NewPos[3*i2],NewPos[3*i2+1],NewPos[3*i2+2]);
		Quat Q1 = Quat(NewQuat[4*e1],NewQuat[4*e1+1],NewQuat[4*e1+2],NewQuat[4*e1+3]);
		...
		diff_un = Quat::FromTwoVectors(d3, xf).normalized();
		un_star = diff_un*Q1;
		qi<<un_star.w(),un_star.x(),un_star.y(),un_star.z();			//J=<0,j1,j2,j3>
		...
		auto debug1 = EigenMatrix2StdVector(CRPosLhs);
		auto debug2 = EigenMatrix2StdVector(CRQuatRhs);
...
void FPDStretchShearConstraints::computeProjection(Vec& NewPos,Vec& NewQuat,const int InConstraintIndex,Vec&CRPosRhs,Vec&CRQuatRhs)
{
	const int32 i = InConstraintIndex;
	{
		const auto& Constraint = MConstraints[i];
		...
		TVector<PDScalar,3>  P1 = TVector<PDScalar,3>(NewPos[3*i1],NewPos[3*i1+1],NewPos[3*i1+2]);
		TVector<PDScalar,3>  P2 = TVector<PDScalar,3>(NewPos[3*i2],NewPos[3*i2+1],NewPos[3*i2+2]);
		Quat Q1 = Quat(NewQuat[4*e1],NewQuat[4*e1+1],NewQuat[4*e1+2],NewQuat[4*e1+3]).normalized();
