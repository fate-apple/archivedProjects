...
namespace Chaos
{
	...
	class FPDAnimationQuatConstraints
	{
	public:
		FPDAnimationQuatConstraints( const CRProperty&  Property,float Weight)
		...
		FPDAnimationQuatConstraints( TArray<TVector<int32, 1>>&& Constraints ,TArray<Quat>&& AnimationQuat,const CRProperty&  Property,float Weight)
			:MConstraints(MoveTemp(Constraints)),MAnimationQuat(MoveTemp(AnimationQuat)),RopeLength(Property.l0)
		{
		...
		FPDAnimationQuatConstraints( TArray<TVector<int32, 1>>&& Constraints ,TArray<Quat>&& AnimationQuat,float Weight)
			:MConstraints(MoveTemp(Constraints)),MAnimationQuat(MoveTemp(AnimationQuat)),WeightCoefInit(0),WeightCoef(Weight)
		{
			check(MConstraints.Num()!=0);
			ComputeLHS();
		}
