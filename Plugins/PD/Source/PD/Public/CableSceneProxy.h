...
public:
	...
	void BuildCableMesh(const TArray<FVector>& InPoints,const TArray<FVector>& InEulers, TArray<FDynamicMeshVertex>& OutVertices, TArray<int32>& OutIndices)
	{
		 FColor VertexColor(255, 255, 255);
		...
		for (int32 PointIdx = 0; PointIdx < NumPoints; PointIdx++)
		{
			...
			for (int32 VertIdx = 0; VertIdx < NumRingVerts; VertIdx++)
			{
				...
				float RadAngle = 2.f * PI * AroundFrac;
				float TexAngle = -fmod(InEulers[PrevIndex][2],PI);
				if(TexAngle<0)TexAngle+=PI;
				TexAngle/= PI;
				...
				auto texcoor = fmod( AroundFrac+TexAngle,1);
				Vert.TextureCoordinate[0] = FVector2D(texcoor, texcoor);
	...
	void SetDynamicData_RenderThread(FRopeDynamicData* NewDynamicData)
	{
		...
		BuildCableMesh(NewDynamicData->Points,NewDynamicData->Eulers,Vertices, Indices);
