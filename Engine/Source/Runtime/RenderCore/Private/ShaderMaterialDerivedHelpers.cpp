	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	Dst.WRITES_CUSTOMDATA_TO_GBUFFER = (Dst.USES_GBUFFER && (Mat.MATERIAL_SHADINGMODEL_SUBSURFACE || Mat.MATERIAL_SHADINGMODEL_PREINTEGRATED_SKIN || Mat.MATERIAL_SHADINGMODEL_SUBSURFACE_PROFILE || Mat.MATERIAL_SHADINGMODEL_CLEAR_COAT || Mat.MATERIAL_SHADINGMODEL_TWOSIDED_FOLIAGE || Mat.MATERIAL_SHADINGMODEL_HAIR || Mat.MATERIAL_SHADINGMODEL_CLOTH || Mat.MATERIAL_SHADINGMODEL_EYE
		//begin sjw modify
		|| Mat.MATERIAL_SHADINGMODEL_TOONSTANDARD || Mat.MATERIAL_SHADINGMODEL_TOONHAIR
		//end sjw modify
		));
	...
	Dst.WRITES_VELOCITY_TO_GBUFFER = ((Dst.SUPPORTS_WRITING_VELOCITY_TO_BASE_PASS || Dst.USES_GBUFFER) && SrcGlobal.GBUFFER_HAS_VELOCITY && (!SrcGlobal.SELECTIVE_BASEPASS_OUTPUTS || !(Lightmap.STATICLIGHTING_TEXTUREMASK || Lightmap.STATICLIGHTING_SIGNEDDISTANCEFIELD || Lightmap.HQ_TEXTURE_LIGHTMAP || Lightmap.LQ_TEXTURE_LIGHTMAP || Lightmap.WATER_MESH_FACTORY)));
	...
	Dst.MOBILE_EMULATION = ((FEATURE_LEVEL == ERHIFeatureLevel::ES3_1) && (!(Compiler.COMPILER_GLSL_ES3_1) && Dst.USE_DEVELOPMENT_SHADERS && (!(Compiler.METAL_PROFILE) || Compiler.MAC) && !Compiler.VULKAN_PROFILE));

	Dst.SUPPORTS_PIXEL_COVERAGE = (FEATURE_LEVEL >= ERHIFeatureLevel::SM5 && !Compiler.COMPILER_GLSL && !Dst.MOBILE_EMULATION);
