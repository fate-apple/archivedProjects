...
void FShaderCompileUtilities::ApplyFetchEnvironment(FShaderMaterialPropertyDefines& SrcDefines, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	// begin sjw modify
	FETCH_COMPILE_BOOL(MATERIAL_SHADINGMODEL_TOONSTANDARD);
	FETCH_COMPILE_BOOL(MATERIAL_SHADINGMODEL_TOONHAIR);
...
static FString CreateGBufferDecodeFunctionDirect(const FGBufferInfo& BufferInfo)
{
	...
	//begin sjw modify
	FullStr += TEXT("\tRet.ToonDataA = InToonDataA;\n");
	//end sjw modify
	FullStr += TEXT("\t\n");
...
static FString CreateGBufferDecodeFunctionVariation(const FGBufferInfo& BufferInfo, EGBufferDecodeType DecodeType, ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	if (DecodeType == CoordUV)
	{
		...
		//begin sjw modify
		//define in scenetexturecommon.ush
		FullStr += TEXT("\tfloat4 ToonDataA = Texture2DSampleLevel(SceneTexturesStruct.ToonBufferATexture, SceneTexturesStruct_ToonBufferATextureSampler, UV, 0).xyzw;\n");
	}
	else if (DecodeType == CoordUInt)
	{
		...
		//begin sjw modify
		FullStr += TEXT("\tfloat4 ToonDataA = SceneTexturesStruct.ToonBufferATexture.Load(int3(PixelPos, 0)).xyzw;\n");
	}
	else if (DecodeType == SceneTextures)
	{
		...
		//begin sjw modify
		//define in DeferredShadingCommon.ush
		FullStr += TEXT("\tfloat4 ToonDataA = ToonBufferATexture.SampleLevel(ToonBufferATextureSampler, UV, 0).xyzw;\n");
	...
	else if (DecodeType == SceneTexturesLoad)
	{
		...
		//begin sjw modify
		FullStr += TEXT("\tfloat4 ToonDataA = ToonBufferATexture.Load(int3(PixelCoord, 0)).xyzw;\n");
	}
	else
	{
		check(0);
	...
	FullStr += TEXT("\n");
	// create by CreateGBufferDecodeFunctionDirect above
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	// begin sjw modify
	if (Mat.MATERIAL_SHADINGMODEL_TOONSTANDARD)
	{
		SetStandardGBufferSlots(Slots, bWriteEmissive, bHasTangent, bHasVelocity, bHasStaticLighting, bIsStrataMaterial);
		Slots[GBS_CustomData] = bUseCustomData;
	}
	if (Mat.MATERIAL_SHADINGMODEL_TOONHAIR)
	{
		SetStandardGBufferSlots(Slots, bWriteEmissive, bHasTangent, bHasVelocity, bHasStaticLighting, bIsStrataMaterial);
		Slots[GBS_CustomData] = bUseCustomData;
	}
