...
//begin sjw modify
#include "Materials/MaterialExpressionOutlineColorOutput.h"
//end sjw modify
...
void UMaterialExpressionBreakMaterialAttributes::Serialize(FStructuredArchive::FRecord Record)
{
	...
	if (UnderlyingArchive.CustomVer(FRenderingObjectVersion::GUID) < FRenderingObjectVersion::FixedLegacyMaterialAttributeNodeTypes)
	{
		...
		//begin sjw modify
		++OutputIndex; Outputs[OutputIndex].SetMask(1, 1, 0, 0, 0);
		++OutputIndex; Outputs[OutputIndex].SetMask(1, 1, 0, 0, 0);
		++OutputIndex; Outputs[OutputIndex].SetMask(1, 1, 1, 1, 0);
	}
#endif // WITH_EDITOR
}
...
static void BuildPropertyToIOIndexMap()
{
	if (PropertyToIOIndexMap.Num() == 0)
	{
		...
		PropertyToIOIndexMap.Add(MP_SpecularOffset, 26);
		PropertyToIOIndexMap.Add(MP_SpecularRange, 27);
		PropertyToIOIndexMap.Add(MP_ShadowColor, 28);
	}
}
