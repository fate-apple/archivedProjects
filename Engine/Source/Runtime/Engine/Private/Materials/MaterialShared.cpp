...
//分配需要编译的ShaderType，Pipline等，就是在这里面把Shader类型分配到ShaderMap里的
...
void FMaterialAttributeDefinitionMap::InitializeAttributeMap()
{
	...
	//begin sjw modify
	AddCustomAttribute(FGuid(0xfcd7b46e, 0xb1234224, 0xbae76b23, 0x639f984c), "OutlineColor", "GetOutlineColor", MCT_Float3, FVector4(0, 0, 1, 0));
	//end sjw modify
}
...
FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	case MP_CustomData0:	
		...
		CustomPinNames.Add({ MSM_ToonStandard, "shadow gray scale" });
		CustomPinNames.Add({ MSM_ToonHair, "Tighten Specular" });
	...
	case MP_CustomData1:
		...
		CustomPinNames.Add({ MSM_ToonStandard, "NOL Offset" });
		CustomPinNames.Add({ MSM_ToonHair, "Scatter" });
	...
	case MP_SpecularRange:
		CustomPinNames.Add({ MSM_ToonStandard, "Specular Range" });
		CustomPinNames.Add({ MSM_ToonSkin, "Specular Range" });
		CustomPinNames.Add({ MSM_ToonHair, "Specular Range" });
		return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "MP_SpecularRange"));
	case MP_SpecularOffset:
		CustomPinNames.Add({ MSM_ToonStandard, "Specular Offset" });
		CustomPinNames.Add({ MSM_ToonSkin, "Specular Offset" });
		CustomPinNames.Add({ MSM_ToonHair, "Specular Offset" });
		return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "MP_SpecularOffset"));
	case MP_ShadowColor:
		CustomPinNames.Add({ MSM_ToonStandard, "Specular ShadowColor" });
		CustomPinNames.Add({ MSM_ToonSkin, "Specular ShadowColor" });
		CustomPinNames.Add({ MSM_ToonHair, "Specular ShadowColor" });
		return FText::FromString(GetPinNameFromShadingModelField(Material->GetShadingModels(), CustomPinNames, "MP_ShadowColor"));
