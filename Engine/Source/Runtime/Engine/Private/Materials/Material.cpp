...
int32 FMaterialResource::CompilePropertyAndSetMaterialProperty(EMaterialProperty Property, FMaterialCompiler* Compiler, EShaderFrequency OverrideShaderFrequency, bool bUsePreviousFrameTime) const
{
	...
	case MP_SpecularOffset:
		Ret = MaterialInterface->CompileProperty(Compiler, MP_SpecularOffset, MFCF_ForceCast);
		break;

	case MP_SpecularRange:
		Ret = MaterialInterface->CompileProperty(Compiler, MP_SpecularRange, MFCF_ForceCast);
		break;

	case MP_ShadowColor:
		Ret = MaterialInterface->CompileProperty(Compiler, MP_ShadowColor, MFCF_ForceCast);
		break;
		//end sjw modify
...
static void AddStrataShadingModelFromMaterialShadingModel(FStrataMaterialInfo& OutInfo, const FMaterialShadingModelField& InShadingModels)
{
	...
	//begin sjw modify
	if (InShadingModels.HasShadingModel(MSM_ToonStandard))	{ OutInfo.AddShadingModel(EStrataShadingModel::SSM_DefaultLit); }
	if (InShadingModels.HasShadingModel(MSM_ToonSkin))	{ OutInfo.AddShadingModel(EStrataShadingModel::SSM_SubsurfaceLit); }
	if (InShadingModels.HasShadingModel(MSM_ToonHair))	{ OutInfo.AddShadingModel(EStrataShadingModel::SSM_Hair); }
}
...
void UMaterial::ConvertMaterialToStrataMaterial()
{
	...
	const bool bRequireNoSubsurfaceProfile	= !bHasShadingModelMixture && (ShadingModel == MSM_Subsurface || ShadingModel == MSM_PreintegratedSkin || ShadingModel == MSM_ToonSkin);
	//end sjw modify
...
void UMaterial::PostLoad()
{
	...
	//begin sjw modify
	DoMaterialAttributeReorder(&SpecularOffset, UEVer, RenderObjVer, UE5MainVer);
	DoMaterialAttributeReorder(&SpecularRange, UEVer, RenderObjVer, UE5MainVer);
	DoMaterialAttributeReorder(&ShadowColor, UEVer, RenderObjVer, UE5MainVer);
	//end sjw modify
...
bool UMaterial::GetExpressionInputDescription(EMaterialProperty InProperty, FMaterialInputDescription& OutDescription)
{
	...
	case MP_ShadingModel: SetMaterialInputDescription(ShadingModelFromMaterialExpression, false, OutDescription); return true;
		//begin sjw modify
	case MP_SpecularOffset: SetMaterialInputDescription(SpecularOffset, false, OutDescription); return true;
	case MP_SpecularRange: SetMaterialInputDescription(SpecularRange, false, OutDescription); return true;
	case MP_ShadowColor: SetMaterialInputDescription(ShadowColor, false, OutDescription); return true;
		//end sjw modify
...
int32 UMaterial::CompilePropertyEx( FMaterialCompiler* Compiler, const FGuid& AttributeID )
{
	...
	switch (Property)
	{
		...
		//begin sjw modify
	case MP_SpecularOffset:			return SpecularOffset.CompileWithDefault(Compiler, Property);
	case MP_SpecularRange:			return SpecularRange.CompileWithDefault(Compiler, Property);
	...
	bool bIsTranslucencyWritingVelocity)
{
	...
	case MP_CustomData0:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ClearCoat, MSM_Hair, MSM_Cloth, MSM_Eye, MSM_SubsurfaceProfile
			/* begin sjw modify*/
			, MSM_ToonStandard, MSM_ToonSkin, MSM_ToonHair
			/* end sjw modify*/
		});
		break;
	case MP_CustomData1:
	...
	case MP_ShadingModel:
		...
		// begin sjw modify
	case MP_SpecularOffset:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ToonStandard, MSM_ToonHair, MSM_ToonSkin});
		break;
	case MP_ShadowColor:
		Active = ShadingModels.HasAnyShadingModel({ MSM_ToonStandard, MSM_ToonHair, MSM_ToonSkin});
		break;
