...
,	TargetPlatform(InTargetPlatform)
{
	...
	//begin sjw modify
	SharedPixelProperties[MP_SpecularOffset] = true;
	SharedPixelProperties[MP_SpecularRange] = true;
...
bool FHLSLMaterialTranslator::Translate()
{
	STAT(double HLSLTranslateTime = 0);
	{
		...
		if (!bEnableExecutionFlow)
		{
			...
			//begin sjw modify
			Chunk[MP_SpecularOffset]				= Material->CompilePropertyAndSetMaterialProperty(MP_SpecularOffset, this);
			Chunk[MP_SpecularRange]					= Material->CompilePropertyAndSetMaterialProperty(MP_SpecularRange, this);
...
void FHLSLMaterialTranslator::GetMaterialEnvironment(EShaderPlatform InPlatform, FShaderCompilerEnvironment& OutEnvironment)
{
	...
	{	
		...
		/* begin sjw modify*/
		if (ShadingModels.HasShadingModel(MSM_ToonStandard))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_TOONSTANDARD"), TEXT("1"));
			NumSetMaterials++;
		}
		if (ShadingModels.HasShadingModel(MSM_ToonHair))
		{
			OutEnvironment.SetDefine(TEXT("MATERIAL_SHADINGMODEL_TOONHAIR"), TEXT("1"));
			NumSetMaterials++;
		}
...
{
	{
		...
		auto debugname = Material->GetFriendlyName();
		bool toonskin = debugname.Contains("ToonSkin", ESearchCase::IgnoreCase);
...
{	
	...
	//begin sjw modify
	LazyPrintf.PushParam(!bEnableExecutionFlow ? *GenerateFunctionCode(MP_SpecularOffset, BaseDerivativeVariation) : TEXT("return 0.0f"));
	LazyPrintf.PushParam(!bEnableExecutionFlow ? *GenerateFunctionCode(MP_SpecularRange, BaseDerivativeVariation) : TEXT("return 0.0f"));
	LazyPrintf.PushParam(!bEnableExecutionFlow ? *GenerateFunctionCode(MP_ShadowColor, BaseDerivativeVariation) : TEXT("return 0.0f"));
