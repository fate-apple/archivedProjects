 ...
 */
//FLumenCardRenderer 包含在 FDeferredShadingSceneRenderer 中，因此包含了整个场景所有的 Card 渲染数据。
class FLumenCardRenderer
{
public:
	//包含用于执行捕获 Material Attribute 的渲染数据
	...
	//MeshDrawCommands 仅用于 Non-Nanite Mesh，并且不是真正的绘制命令，而是用于执行可见性处理的命令列表，这是为了适配 Mesh Draw pipeline 流程。此列表根据 CardPagesToRender 生成。
