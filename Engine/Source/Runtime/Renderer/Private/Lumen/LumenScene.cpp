...
void UpdateLumenScenePrimitives(FScene* Scene)
{
	...
	// Remove primitives
	//删除 FLumenPrimitiveGroup 过程中会删除对应的 MeshCards，以及 MeshCards 包含的 Card 对象
	{
		...
		//对删除过Primitive 的group删除对应的 MeshCards，以及 MeshCards 包含的 Card 对象，其中最为关键的是释放 Surface Cache.
		//如果已空， 删除group
	...
	// Add primitives
	//根据 FPrimitiveSceneInfo 创建 FLumenPrimitiveGroup 对象
	//Instance Merge，ISM 会在此步骤中尝试进行合并，合并不一定成功，具体取决于所有 Instance Bounding 累计表面积和合并后总的 Bounding 表面积的比例，尽可能最大化 Altas 空间利用率
	{
		...
		for (FPrimitiveSceneInfo* ScenePrimitiveInfo : LumenSceneData.PendingAddOperations)
		{
			...
			//RayTracingGroupId 有效：
			//1.根据 SceneProxy 获取 RayTracingGroupID，根据此 ID 获取对应的 LumenPrimitiveGroup。
			if (bAnyInstanceValid)
			{
					...
					&& !SceneProxy->IsEmissiveLightSource())
				{
					...
					//2.如果 LumenPrimitiveGroup 已存在则删除其对应的 LumenMeshCards 的所有 Card，
					//将当前 PrimitiveSceneInfo 加入到 PrimitiveGroup 的 Primitives 列表中，然后遍历 Primitives 列表，合并所有 Primitive 对应的 SceneProxy 的 BBox。
					...
					//如果 LumenPrimitiveGroup 不存在则增加一个新的 LumenPrimitiveGroup
					else
					{
				...
				//SceneProxy 未指定 RayTracingGroupId：
				//1.如果有多于一个以上的 Instance 且启用合并 Instance（GLumenMeshCardsMergeInstances 控制变量），则遍历每个 Instance 累计每个 Bound 的面积和，使用所有 Instance 总的 Bound 的面积除以面积和，
				//如果这个比值小于 GLumenMeshCardsMergeInstancesMaxSurfaceAreaRatio 控制变量，则执行合并，只增加一个 FLumenPrimitiveGroup，
				//设置 PrimitiveInstanceIndex 为 -1，设置 WorldSpaceBoundingBox 为所有 Instance 合并后的 World BBox
				else
				{
					...
					if (NumInstances > 1)
					{
						...
						//如果没有执行合并，则遍历每个 Instance 增加 FLumenPrimitiveGroup，
						//设置 PrimitiveInstanceIndex 为 InstanceIndex，设置 WorldSpaceBoundingBox 为每个 Instance 的 World BBox。
						if (!bMergedInstances)
						{
					...
					//如果只有一个 Instance，则增加 FLumenPrimitiveGroup，设置 PrimitiveInstanceIndex 为 0，设置 WorldSpaceBoundingBox 为所有 SceneProxy 的 World BBox
					else
					{
						const int32 PrimitiveGroupIndex = LumenSceneData.PrimitiveGroups.AddSpan(1);
						ScenePrimitiveInfo->LumenPrimitiveGroupIndices.Add(PrimitiveGroupIndex);
		...
		//这一步仅仅是创建 FLumenPrimitiveGroup，并没有创建 MeshCards、Card 以及分配 PageTable，这些都是在后续步骤中完成。
	}

	// UpdateLumenPrimitives
	//遍历每个 PendingUpdateOperations，遍历其对应的所有 FLumenPrimitiveGroup，如果是 Instance，则更新 Instance 的 WorldSpaceBoundingBox，
	//并调用 UpdateMeshCards 函数更新 FLumenPrimitiveGroup 对应的 LumenMeshCards 所有的 Card 的 Transform
	{
...
//释放 Surface Cache
//历每个 Card 的 Surface Mipmap，释放其所包含的每个 Virtual Page，回收在 PageTable 中对应的 FLumenPageTableEntry
