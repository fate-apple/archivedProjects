...
#define IMPLEMENT_VIRTUALTEXTURE_SHADER_TYPE(PolicyType, PolicyName) \
		...
		ERuntimeVirtualTextureDebugType DebugType)
	{
		...
		// Build graph
		//FRenderGraphSetup 根据 RVT Asset 配置的内容生成 RDG 资源，比如选择 底色、法线、粗糙度、高光度 会生成 3 张 B8G8R8A8 格式的 RenderTarget，用于生成 Diffuse、Normal、Specular/Roughness 数据。
		//FRenderGraphSetup 包含了 3 个 Pass 所需要的所有 RGD 资源，并且为了节省 GPU 内存，使用了重叠资源
		...
		if (GraphSetup.bRenderPass)
		{
	    	...
	    	{
	    		// GatherMeshesToDraw 搜集所有在当前 RVT Volume 范围内的 Mesh 进行绘制。对应绘制的 Shader 是 VirtualTextureMaterial.usf，在 PS 中通过 MRT 输出到对应的 RT 中
