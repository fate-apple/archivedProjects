...
)
{
...

	{
		...
		{
			// 被每类pass， 当DynamicMeshElementsPassRelevance(passMask) 包含该类pass才添加
			if (!DynamicMeshElementsPassRelevance || (*DynamicMeshElementsPassRelevance)[MeshIndex].Get(PassType))
			{
				// from ViewInfo View.DynamicMeshElements
...

	{
		...
		{
			// come from SetupMeshPass-> Pass.DispatchPassSetup(... ViewCommands.DynamicMeshCommandBuildRequests[PassType]...), for each EMeshPass type
...
{
public:
	//com from FParallelMeshDrawCommandPass.TaskContext etc.
