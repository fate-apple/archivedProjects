...
public:
	...
	virtual void DrawMesh(const FMeshBatch& Mesh, float ScreenSize) final override
	{
		if (Mesh.HasAnyDrawCalls())
		{
			...
			bool bUseAnisotropy = Material.GetShadingModels().HasAnyShadingModel({MSM_DefaultLit, MSM_ClearCoat, MSM_ToonHair}) && Material.MaterialUsesAnisotropy_RenderThread();
			...
			//begin sjw info
			bool bUseOutline = Material.GetShadingModels().HasAnyShadingModel({MSM_ToonStandard, MSM_ToonSkin ,MSM_ToonHair }) ;
			//end sjw modify
