...
namespace Obi
{
    public class BurstStretchShearConstraintsBatch : BurstConstraintsBatchImpl, IStretchShearConstraintsBatchImpl
    {
        ...
        public override JobHandle Evaluate(JobHandle inputDeps, float stepTime, float substepTime, int substeps)
        {
            ...
            {
                inputDeps.Complete();
                var _lambdas = lambdas.Reinterpret<float, float3>();

                var positions = solverImplementation.positions;
                var orientations = solverImplementation.orientations;
                var invMasses = solverImplementation.invMasses;
                var invRotationalMasses = solverImplementation.invRotationalMasses;

                var deltas = solverImplementation.positionDeltas;
                var orientationDeltas = solverImplementation.orientationDeltas;
                var counts = solverImplementation.positionConstraintCounts;
                var orientationCounts = solverImplementation.orientationConstraintCounts;

                var deltaTimeSqr = substepTime * substepTime;
