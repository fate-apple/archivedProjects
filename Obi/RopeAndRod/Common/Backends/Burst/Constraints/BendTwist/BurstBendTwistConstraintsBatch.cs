    ...
    public class BurstBendTwistConstraintsBatch : BurstConstraintsBatchImpl, IBendTwistConstraintsBatchImpl
    {
        ...
        public override JobHandle Evaluate(JobHandle inputDeps, float stepTime, float substepTime, int substeps)
        {
            if (Global.debugMode)
            {
                ...
                for (int i = 0; i < m_ConstraintCount; i++)
                {
                    ...
                    float3 dlambda = (omega.value.xyz - compliances * _lambdas[i]) / (compliances + new float3(w1 + w2 + BurstMath.epsilon));
        ...
        public override JobHandle Apply(JobHandle inputDeps, float substepTime)
        {
            ...
            if (Global.debugMode)
            {
                ...
                var orientationDeltas = solverImplementation.orientationDeltas;
                var orientationCounts = solverImplementation.orientationConstraintCounts;
                var sorFactor = parameters.SORFactor;
                for (int i = 0; i < m_ConstraintCount; i++)
                {
