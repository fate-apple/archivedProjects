...
namespace MathNet.Numerics.LinearAlgebra.Double.Factorization
{
    ...
    public class SparseLU : ISolver<double>
    {
            ...
            double tol = 1.0)
        {
            ...
            var debug =  (SparseCompressedRowMatrixStorage<double>)matrix.Transpose().Storage;
            ...
            if (dm.StructuralRank < n)
            {
                //throw new NotImplementedException();
                Debug.LogError(String.Format( "rank {0} < dimension ",dm.StructuralRank));
            }

            var lu = CSparseLU.Create(A, ordering, tol);
            return new SparseLU(lu, n);
        }
