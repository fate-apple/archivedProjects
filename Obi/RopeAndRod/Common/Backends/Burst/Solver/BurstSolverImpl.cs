...
namespace Obi
{
    ...
    public class BurstSolverImpl : ISolverImpl
    {
        ...
        //TODO: Parameters For PD
        public int PDIterations=5;
        IBurstPDConstraintsImpl[] PDConstraints;
        public SparseMatrix CRPosLhs;
        public SparseMatrix CRQuatLhs;
       
        public Vec CRPosRhs;
        public Vec CRQuatRhs;
        public Vec PlainPos;
        public Vec PlainQuat;
        ...
        public BurstSolverImpl(ObiSolver solver)
        {
            ...
            constraints[(int)Oni.ConstraintType.PDStretchShear] = new BurstPDStretchShearConstraints(this);
            constraints[(int)Oni.ConstraintType.PDBendTwist] = new BurstPDBendTwistConstraints(this);
        ...
        {
            float r4 = math.pow(abstraction.PDCRProperty.R, 4);
            var updateInertiaTensors = new UpdateInertiaTensorsJob()
            {
                ...
                inertiaTensors = abstraction.InertiaTensors.AsNativeArray<float4>(),
                p4Radii = r4
        ...
        public void ParticleCountChanged(ObiSolver solver)
        {
            ...
            //Masses = abstraction.Masses.AsNativeArray<float>();
            Masses = new NativeArray<float>(invMasses.Length, Allocator.Persistent);
            ...
            //TODO: just init when needed
            // CRPosRhs =  Vec.Build.Dense(3*positions.Length,i=>0f);
            // CRQuatRhs =  Vec.Build.Dense(4*(orientations.Length-1),i=>0f);
            // CRPosLhs =  SparseMatrix.Create(3*positions.Length,3*positions.Length,0);
            // CRQuatLhs =  SparseMatrix.Create(4*(orientations.Length-1),4*(orientations.Length-1),0);
            //TODO: now update in  RecalculateInertiaTensors()
            InertiaTensors = abstraction.InertiaTensors.AsNativeArray<float4>();
            var oritentationCount = orientations.Length;
            if(!Global.particleOritentation) 
                oritentationCount = math.max( orientations.Length-1,0);
            PlainPos = Vec.Build.Dense(positions.Length * 3, 0);
            PlainQuat =Vec.Build.Dense((oritentationCount) * 4, 0);
            //TODO[sjw]
            propery = abstraction.PDCRProperty;
            var uniformMass = propery.Density * (math.PI * propery.R * propery.R * propery.totalLength) / particleCount;
            for (int i = 0; i < particleCount; i++)
            {
                //Masses[i] = 1 / (invMasses[i]+BurstMath.epsilon);
                Masses[i] = uniformMass;
            }
        }
        ...
        public IObiJobHandle Substep(float stepTime, float substepTime, int substeps)
        {
            ...
            JobHandle projectionHandle;
            if (abstraction.PDCRProperty.enable)
            {
                UnityEngine.Profiling.Profiler.BeginSample("Project");
                //var preProcessHandle = ApplyConstraints(predictPositionsHandle, stepTime, substepTime, substeps);
                JobHandle PDHandle = PDProjectConstraints(predictPositionsHandle, stepTime, substepTime, substeps);
                projectionHandle = ApplyConstraints(PDHandle, stepTime, substepTime, substeps);
                UnityEngine.Profiling.Profiler.EndSample();
            }
            else { projectionHandle = ApplyConstraints(predictPositionsHandle, stepTime, substepTime, substeps);}
            if(Global.debugMode)
            ...
            if(Global.debugMode)
                velocityCorrectionsHandle.Complete();
            // Update positions:
        ...
        { 
            ...
            {
                Debug.Log(string.Format("{0}:{1} {2} {3}",i,positions[i].x,positions[i].y,positions[i].z));
            }
        }
        ...
        private JobHandle ApplyConstraints(JobHandle inputDeps, float stepTime, float substepTime, int substeps)
        {
            ...
            if (Global.debugMode)
            {
                inputDeps.Complete();
                                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                                             int maxIterations = 0;
            for (int i = 0; i < Oni.PBDConstraintTypeCount; ++i)
            {
            ...
            int[] padding = new int[Oni.PBDConstraintTypeCount];
            for (int i = 0; i < Oni.PBDConstraintTypeCount; ++i)
            {
                var parameters = m_Solver.GetConstraintParameters((Oni.ConstraintType)i);
            ...
            {
                for (int j = 0; j < Oni.PBDConstraintTypeCount; ++j)
                {
        ...
        private bool shouldRebuildMatrix = true;
        //private Svd<double> svdCRPosLhs,svdQuatLhs;
        private SparseLU svdCRPosLhs,svdQuatLhs;
        public SparseMatrix MatrixMass,MatrixJ;
        public Vec PosRhsCached;
        public NativeArray<float4> lJ;
        private JobHandle PDProjectConstraints(JobHandle inputDeps, float stepTime, float substepTime, int substeps)
        {
            ...
            {
                //UnityEngine.Profiling.Profiler.BeginSample("shouldRebuildMatrix");
                // CRPosLhs.SetToZero();
                // CRQuatLhs.SetToZero();
                // PlainPos = Vector.Create<float>(positions.Length * 3, 0);
                // PlainQuat =Vector.Create<float>(orientations.Length * 4, 0);
                PlainPos = Vec.Build.Dense(positions.Length * 3, 0);
                PlainQuat =Vec.Build.Dense((oritentationCount) * 4, 0);
                Solver2Plain(inputDeps);
                
                CRPosLhs =  SparseMatrix.Create(3*positions.Length,3*positions.Length,0);
                for (int i = 0; i < particleCount; i++)
                {
                    CRPosLhs[3 * i,3*i] = Masses[i];
                    CRPosLhs[3 * i + 1,3 * i + 1] = Masses[i];
                    CRPosLhs[3 * i + 2,3 * i + 2] = Masses[i];
                }
                MatrixMass = CRPosLhs;
                //CRPosLhs.MultiplyInPlace( (1 / (substepTime * substepTime)));
                CRPosLhs *= (1 / (substepTime * substepTime));
                CRPosRhs = CRPosLhs.Multiply(PlainPos);
                
                //CRQuatLhs =  Matrix.CreateSparse<float>(4*orientations.Length,4*orientations.Length);
                CRQuatLhs =  SparseMatrix.Create (PlainQuat.Count,PlainQuat.Count,0);
                NativeArray<float4> LJ = new NativeArray<float4>(InertiaTensors.Length,Allocator.Temp);
                NativeArray<float4>.Copy(InertiaTensors,LJ);
                //TODO[sjw]: refactor to somewhere else 
                if (!Global.uniformSampled)
                {
                    var batches =
                        (constraints[(int)Oni.ConstraintType.PDStretchShear] as
                            BurstConstraintsImpl<BurstPDStretchShearConstraintsBatch>)?.batches;
                    for (int i = 0; i < batches.Count; ++i)
                        if (batches[i].enabled)
                        {
                            batches[i].MultiplyLJ(ref LJ);
                        }
                }
              
                for (int i = 0; i < oritentationCount; i++)
                {
                    CRQuatLhs[4 * i, 4 * i] = LJ[i].w ;
                    CRQuatLhs[4 * i + 1, 4 * i + 1] = LJ[i].x ;
                    CRQuatLhs[4 * i + 2, 4 * i + 2] = LJ[i].y ;
                    CRQuatLhs[4 * i + 3, 4 * i + 3] = LJ[i].z ;
                }
                // J *=lp
                CRQuatLhs *=  abstraction.PDCRProperty.Density;
                if(Global.uniformSampled)
                    CRQuatLhs *= abstraction.PDCRProperty.restLength;
                
                   
                MatrixJ = CRQuatLhs;        //Cached for re-compute rhs
                //CRQuatLhs.MultiplyInPlace( (1 / (substepTime * substepTime))* abstraction.PDCRProperty.Density);
                CRQuatLhs*=  (1 / (substepTime * substepTime));
                CRQuatRhs = CRQuatLhs.Multiply(PlainQuat);
                
                //var debug1 = Matirx2DebugList(CRPosLhs);
                for (int j = Oni.PBDConstraintTypeCount; j < Oni.ConstraintTypeCount; ++j)
                {
                    switch (j)
                    {
                        case (int)Oni.ConstraintType.PDStretchShear:
                            inputDeps = (constraints[j] as BurstConstraintsImpl<BurstPDStretchShearConstraintsBatch>)
                                .RebuildPDMatrix(inputDeps);
                            break;
                        case (int)Oni.ConstraintType.PDBendTwist:
                            inputDeps = (constraints[j] as BurstConstraintsImpl<BurstPDBendTwistConstraintsBatch>)
                                .RebuildPDMatrix(inputDeps);
                            break;
                        default:
                ...
                PosRhsCached = Vec.Build.Dense(particleCount*3,0);
                if (Global.animationConstrain)
                {
                    for (int i = 0; i < particleCount; i++)
                    {
                        if (invMasses[i] == 0)
                        {
                            var PosA = SparseMatrix.CreateIdentity(3);
                            SparseMatrix PosSelectionMatrix = SparseMatrix.Create(3, 3 * particleCount, 0);
                            PosSelectionMatrix[0, 3 * i] = 1;
                            PosSelectionMatrix[1, 3 * i + 1] = 1;
                            PosSelectionMatrix[2, 3 * i + 2] = 1;
                            var WeightCoef = propery.E * propery.A * propery.totalLength * particleCount;
                            CRPosLhs += (SparseMatrix)(WeightCoef * PosSelectionMatrix.Transpose() * PosA.Transpose() *
                                                       PosA *
                                                       PosSelectionMatrix);
                        }
                    }
                ...
                if (Global.debugMode)
                {
                    var debug1 = CRPosLhs.ToArray();
                    var debug2 = CRQuatLhs.ToArray();
                }
                svdCRPosLhs  =  CRPosLhs.SparseLU(1);
                svdQuatLhs =CRQuatLhs.SparseLU(1);
                // svdCRPosLhs = CRPosLhs.Svd();
                // svdQuatLhs = CRQuatLhs.Svd();
                shouldRebuildMatrix = false;
            }

            Solver2Plain(inputDeps);
            for (int k = 0; k < abstraction.PDCRProperty.PdIteration; k++)
            {
                UnityEngine.Profiling.Profiler.BeginSample("ReInitRHS");
                if (Global.debugMode)
                {
                    var debug1 = MatrixMass.ToArray();
                    var debug2 = MatrixJ.ToArray();
                    var debug3 = MatrixJ.Multiply(Vector.Build.Dense(PlainQuat.Count, 0.7));
                    var debug4 = MatrixJ.Multiply(PlainQuat);
                }
                CRPosRhs = MatrixMass.Multiply(PlainPos);
                //CRPosRhs.MultiplyInPlace(1 / (substepTime * substepTime));
                CRPosRhs*= (1 / (substepTime * substepTime));
                CRQuatRhs = MatrixJ.Multiply(PlainQuat);
                //CRQuatRhs.MultiplyInPlace(1 / (substepTime * substepTime));
                CRQuatRhs*=(1 / (substepTime * substepTime));
                CRPosRhs += PosRhsCached;
                if (Global.animationConstrain)
                {
                    for (int i = 0; i < particleCount; i++)
                    {
                        if (invMasses[i] == 0)
                        {
                            ...
                            PosSelectionMatrix[2, 3 * i + 2] = 1;
                            var WeightCoef = propery.E * propery.A * propery.totalLength * particleCount;
                            CRPosRhs += (WeightCoef * PosSelectionMatrix.Transpose() * PosA.Transpose()) *
                                        Vec.Build.DenseOfArray(
                                            new double[] { positions[i].x, positions[i].y, positions[i].z });
                            
                        }
                    }
                }
                UnityEngine.Profiling.Profiler.EndSample();
                UnityEngine.Profiling.Profiler.BeginSample("PDProject");
                for (int j = Oni.PBDConstraintTypeCount; j < Oni.ConstraintTypeCount; ++j)
                {
                    inputDeps = constraints[j].Project(inputDeps, stepTime, substepTime, substeps);
                }
                UnityEngine.Profiling.Profiler.EndSample();
                UnityEngine.Profiling.Profiler.BeginSample("PDSolve");
                if (Global.debugMode)
                {
                    var debug1 = Matirx2DebugList(CRPosLhs);
                    var debug2 = CRQuatLhs.ToArray();
                    var debug3 = CRPosLhs.ToArray();
                    var debug4 = CRQuatRhs.ToArray();
                }
                PlainPos = svdCRPosLhs.Solve(CRPosRhs);
                PlainQuat = svdQuatLhs.Solve(CRQuatRhs);
               
                inputDeps.Complete();
                UnityEngine.Profiling.Profiler.EndSample();
                //TODO[sjw] may further optimize,now write back every iteration because job use NavieArray only.
                UnityEngine.Profiling.Profiler.BeginSample("Write Back");
                inputDeps = Plain2Solver(inputDeps);
                inputDeps.Complete();
                UnityEngine.Profiling.Profiler.EndSample();
            }
        ...
        //TODO: maybe JobHandle 
         public JobHandle Plain2Solver(JobHandle inputDeps)
         {
             ...
             {
                 var index = 3 * i;
                 if(Global.particleOritentation&&invMasses[i]==0)
                 {
                     PlainPos[index] = positions[i].x;
                     PlainPos[index+1] = positions[i].y;
                     PlainPos[index+2] = positions[i].z;
                 }
                 else 
                     positions[i] = (float4)new double4(PlainPos[index], PlainPos[index + 1], PlainPos[index + 2], positions[i].w);
             }

             i = 0;
             for (; i < (PlainQuat.Count)/4; i++)
             {
                 if(Global.particleOritentation&& invRotationalMasses[i]==0) 
                 {
                     PlainQuat[4 * i] = orientations[i].value.w;
                     PlainQuat[4 * i+1] = orientations[i].value.x;
                     PlainQuat[4 * i+2] = orientations[i].value.y;
                     PlainQuat[4 * i+3] = orientations[i].value.z;
                 }
                 else
                     orientations[i] = math.normalize( new quaternion((float)PlainQuat[4*i+1],(float)PlainQuat[4*i+2], (float)PlainQuat[4*i+3], (float)PlainQuat[4*i]));
             }
             i--;
             orientations[i+1] = math.normalize( new quaternion((float)PlainQuat[4*i+1],(float)PlainQuat[4*i+2], (float)PlainQuat[4*i+3], (float)PlainQuat[4*i]));

             return inputDeps;
         }
         public JobHandle Solver2Plain(JobHandle inputDeps)
         {
             inputDeps.Complete();
             for (int i = 0; i < positions.Length; i++)
             {
             ...
             for (int i = 0; i < PlainQuat.Count/4; i++)
             {
                 var index = 4 * i;
                 PlainQuat[index] = orientations[i].value.w;
                 PlainQuat[index+1] = orientations[i].value.x;
                 PlainQuat[index+2] = orientations[i].value.y;
                 PlainQuat[index+3] = orientations[i].value.z;
             }
         ...
         public List<List<double>> Matirx2DebugList(SparseMatrix m)
         {
