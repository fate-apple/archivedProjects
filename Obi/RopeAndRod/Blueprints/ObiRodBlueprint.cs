...
namespace Obi
{
    ...
    public class ObiRodBlueprint : ObiRopeBlueprintBase
    {
        ...
        protected override IEnumerator Initialize()
        {
            ...
            IEnumerator PDdc = CreatePDStretchShearConstraints(particleNormals);

            while (PDdc.MoveNext())
                yield return PDdc.Current;
            // Create bending constraints:
            IEnumerator bc = CreateBendTwistConstraints();

            while (bc.MoveNext())
                yield return bc.Current;
            IEnumerator PDbc = CreatePDBendTwistConstraints();
        ...
        protected virtual IEnumerator CreatePDBendTwistConstraints()
        {
            PDbendTwistConstraintsData = new ObiPDBendTwistConstraintsData();

            // Add two batches:
            PDbendTwistConstraintsData.AddBatch(new ObiPDBendTwistConstraintsBatch());
            PDbendTwistConstraintsData.AddBatch(new ObiPDBendTwistConstraintsBatch());
            ...
            {
                if(!Global.particleOritentation&&i+1==totalParticles - 1) 
                    break;
                var batch = PDbendTwistConstraintsData.batches[i % 2] as ObiPDBendTwistConstraintsBatch;
                ...
                batch.AddConstraint(indices, darboux,restLengths[i]);
        ...
        protected virtual IEnumerator CreatePDStretchShearConstraints(List<Vector3> particleNormals)
        {
            PDstretchShearConstraintsData = new ObiPDStretchShearConstraintsData();

            PDstretchShearConstraintsData.AddBatch(new ObiPDStretchShearConstraintsBatch());
            PDstretchShearConstraintsData.AddBatch(new ObiPDStretchShearConstraintsBatch());
            ...
            for (int i = 0; i < totalParticles - 1; i++)
            {
                ...
                var batch = PDstretchShearConstraintsData.batches[i % 2] as ObiPDStretchShearConstraintsBatch;
